...
仅记录本审计确证的冲突和会改变产品边界/成本/安全/不可逆架构的未决项。

| Decision ID | 问题 | 白皮书依据 | 仓库现状 | 推荐默认方案 | 影响 | 是否阻断下一阶段 |
|---|---|---|---|---|---|---|
| DEC-CONFLICT-001 | v35 使用“复合玄学/命理/数字灵魂”定位；V2 明确隔离 destiny/birth 信息并禁止命理式产品 | 第 1–4 章，特别第 2 章 | `docs/FUTURE_SIMULATOR_V2.md` 的 Destiny Isolation；`AGENTS.md` 禁止 astrology/fortune-telling | 对外产品与 Alpha 默认采用 V2 的“现实证据、明确假设、情景沙盘”定位；保留冲突，不改写 v35 | 品牌、输入、文案、模型边界 | **是：阻断面向用户的 Alpha 文案/输入定稿；不阻断 Phase 1 数据地基** |
...
下一阶段没有创始人决策阻断：可先执行 Phase 1“受控数据地基”。但任何面向用户的定位、输入文案、外部邀请或 Alpha 公开描述必须先解决 `DEC-CONFLICT-001`。
