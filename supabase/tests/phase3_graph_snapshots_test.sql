...
select ok(exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'relation_edges' and policyname = 'relation_edges_select_phase4_owner_chain' and qual ilike '%seed_contexts%' and qual ilike '%relation_graph_snapshots%'), 'Edge owner RLS policy preserves the submitted Seed and canonical Graph chain');
...
select ok(exists (select 1 from pg_constraint where conrelid = to_regclass('public.relation_edges') and conname = 'relation_edges_graph_full_binding_fkey'), 'Edge parent owner Seed request and safety binding is a composite foreign key');
