...
select is((select count(*) from public.seed_contexts), 16::bigint, 'existing 16 Seed rows are preserved');
select is((select count(*) from public.consent_events), 16::bigint, 'existing 16 Consent rows are preserved');
select is((select count(*) from supabase_migrations.schema_migrations), 13::bigint, 'exactly one forward M1 security migration follows the canonical 12');
