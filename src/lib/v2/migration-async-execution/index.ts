...
export interface AsyncExecutionRuntimeV2 {
  nowEpochMs(): number;
}
...
type RuntimeTimeRead = { ok: true; epochMs: number; iso: string } | { ok: false; errorCode: "invalid_runtime_time" };
function readOperationNow(runtime: AsyncExecutionRuntimeV2): RuntimeTimeRead {
  try {
    const epochMs = runtime.nowEpochMs();
    if (!Number.isSafeInteger(epochMs) || epochMs < 0 || epochMs > 8.64e15) return { ok: false, errorCode: "invalid_runtime_time" };
    return { ok: true, epochMs, iso: new Date(epochMs).toISOString() };
  } catch { return { ok: false, errorCode: "invalid_runtime_time" }; }
}
...
const canonicalResultBundleSchema = z.object({
  stage2RealityBoundary: requiredUnknown,
  stage3World: requiredUnknown,
  stage4: z.object({ runSpec: requiredUnknown, trajectories: z.array(z.unknown()) }).strict(),
  stage5Analysis: requiredUnknown,
  stage6: z.object({ claimSet: requiredUnknown, claims: z.array(z.unknown()), report: requiredUnknown }).strict(),
  stage7: stage7BundleSchema,
}).strict();
...
function parseCanonicalResultBundle(input: unknown): CanonicalStage2To7ResultBundleV2 | null {
  try {
    const parsed = canonicalResultBundleSchema.safeParse(input);
    return parsed.success ? clone(parsed.data as CanonicalStage2To7ResultBundleV2) : null;
  } catch { return null; }
}
...
async function validateBundle(bundle: CanonicalStage2To7ResultBundleV2, job: AsyncSimulationJobV2, outcomeRepository: OutcomeCalibrationRepositoryPortV2): Promise<ResolvedCanonicalBundleV2 | null> {
  try {
    const parsedBundle = parseCanonicalResultBundle(bundle); if (!parsedBundle) return null;
    const boundary = parseRealityBoundarySnapshotV2(parsedBundle.stage2RealityBoundary);
    if (!boundary.ok || !validateWorldV2(parsedBundle.stage3World).ok) return null;
    const spec = parseBatchRunSpecV2(parsedBundle.stage4.runSpec); if (!spec.ok || canonical(spec.value) !== canonical(job.runSpec) || spec.value.seedContextId !== job.seedContext.id) return null;
    const world = parsedBundle.stage3World as { realityBoundarySnapshot?: unknown; seedContextId?: string; realityBoundaryRevisionSnapshot?: number };
    ...
    if (parsedBundle.stage4.trajectories.length !== spec.value.trajectorySeeds.length || parsedBundle.stage4.trajectories.some((trajectory, index) => !parseTrajectoryResultForFeatureV2(spec.value.trajectoryTemplate.initialWorld, trajectory, { seedContextId: spec.value.seedContextId, trajectorySeed: spec.value.trajectorySeeds[index], policyId: spec.value.policyId, policyVersion: spec.value.policyVersion, trajectoryEngineVersion: spec.value.trajectoryEngineVersion, batchRunSpec: spec.value }).ok)) return null;
    const rebuiltAnalysis = revalidateBatchAnalysisV2({ ...parsedBundle.stage5Analysis, spec: spec.value, trajectories: parsedBundle.stage4.trajectories }, boundary.boundary);
    if (!rebuiltAnalysis.ok || canonical(rebuiltAnalysis.analysis) !== canonical(parsedBundle.stage5Analysis)) return null;
    const claimSet = parsedBundle.stage6.claimSet as { kind?: unknown; payload?: unknown; realityBoundary?: unknown };
    ...
    const claims = buildClaimsV2(parsedBundle.stage6.claimSet);
    if (!claims.ok || canonical(claims.claims) !== canonical(parsedBundle.stage6.claims) || !validateClaimsReportV2(parsedBundle.stage6.report, parsedBundle.stage6.claimSet, parsedBundle.stage6.claims).ok) return null;
    const persisted = await resolvePersistedForecastLock(parsedBundle.stage7.forecastLockReference, outcomeRepository);
    ...
    if (canonical(lock.sourceSnapshots.run) !== canonical({ kind: "batch", payload: rebuiltAnalysis.analysis }) || canonical(lock.sourceSnapshots.claimSet) !== canonical(parsedBundle.stage6.claimSet) || canonical(lock.sourceSnapshots.claims) !== canonical(parsedBundle.stage6.claims) || canonical(lock.sourceSnapshots.report) !== canonical(parsedBundle.stage6.report) || canonical(lock.sourceSnapshots.claimSet.realityBoundary) !== canonical(boundary.boundary)) return null;
    return persisted;
  } catch { return null; }
}
...
type LeaseAuthorityInputV2 = { jobId: string; workerId: string; leaseToken: string; attempt: number };
function isOperationTimeAtOrAfterJobTimeline(job: AsyncSimulationJobV2, operationNowEpochMs: number) {
  return [job.createdAt, job.startedAt, job.leasedAt, job.completedAt].every((time) => {
    if (time === null) return true;
    const epochMs = Date.parse(time);
    return Number.isSafeInteger(epochMs) && epochMs <= operationNowEpochMs;
  });
}
function hasLeaseAuthority(job: AsyncSimulationJobV2 | undefined, input: LeaseAuthorityInputV2, operationNowEpochMs: number) {
  return Boolean(job && job.status === "running" && job.jobId === input.jobId && job.workerId === input.workerId && job.leaseToken === input.leaseToken && job.attempt === input.attempt && isOperationTimeAtOrAfterJobTimeline(job, operationNowEpochMs) && Date.parse(job.leaseExpiresAt ?? "") > operationNowEpochMs);
}
...
export type AsyncSimulationJobRepositoryPortV2 = { submit(input: unknown): Promise<Ok<AsyncSimulationJobV2> & { idempotent: boolean } | Fail<"invalid_job_input" | "idempotency_conflict" | "invalid_runtime_time">>; get(input: unknown): Promise<Ok<AsyncSimulationJobV2 | null> | Fail<"invalid_job_input">>; claim(input: unknown): Promise<Ok<AsyncSimulationJobV2 | null> | Fail<"invalid_worker_input" | "invalid_runtime_time">>; complete(input: unknown): Promise<Ok<AsyncSimulationJobV2> | Fail<"invalid_completion_input" | "invalid_runtime_time" | "lease_mismatch" | "invalid_canonical_artifacts" | "result_binding_tampering">>; fail(input: unknown): Promise<Ok<AsyncSimulationJobV2> | Fail<"invalid_completion_input" | "invalid_runtime_time" | "lease_mismatch">>; };
export function createInMemoryAsyncSimulationJobRepositoryV2(outcomeRepository: OutcomeCalibrationRepositoryPortV2, runtime: AsyncExecutionRuntimeV2): AsyncSimulationJobRepositoryPortV2 {
  const jobs = new Map<string, AsyncSimulationJobV2>(); const keys = new Map<string, { fingerprint: string; jobId: string }>();
  return {
    async submit(input) { try { const parsed = submitSchema.safeParse(input); if (!parsed.success) return fail("invalid_job_input"); const spec = parseBatchRunSpecV2(parsed.data.runSpec); if (!spec.ok || spec.value.seedContextId !== parsed.data.seedContext.id) return fail("invalid_job_input"); const requestFingerprint = fingerprint({ seedContext: parsed.data.seedContext, runSpec: spec.value, schemaVersion: parsed.data.schemaVersion }); const existing = keys.get(parsed.data.idempotencyKey); if (existing) return existing.fingerprint === requestFingerprint ? { ok: true, data: clone(jobs.get(existing.jobId)!), errorCode: null, idempotent: true } : fail("idempotency_conflict"); const operationNow = readOperationNow(runtime); if (!operationNow.ok) return fail("invalid_runtime_time"); const jobId = `async_simulation_job_v2_${fingerprint({ requestFingerprint, key: parsed.data.idempotencyKey })}` as const; const job = signedJob({ jobId, requestFingerprint, seedContext: clone(parsed.data.seedContext), runSpec: clone(spec.value), schemaVersion: "2.0", engineVersion: spec.value.trajectoryEngineVersion, status: "queued", attempt: 0, createdAt: operationNow.iso, startedAt: null, completedAt: null, workerId: null, leaseToken: null, leasedAt: null, leaseExpiresAt: null, resultIds: null, errorCode: null, resultBindingIntegritySignature: null }); jobs.set(jobId, job); keys.set(parsed.data.idempotencyKey, { fingerprint: requestFingerprint, jobId }); return { ok: true, data: clone(job), errorCode: null, idempotent: false }; } catch { return fail("invalid_job_input"); } },
    ...
    async claim(input) { try { const parsed = workerSchema.safeParse(input); if (!parsed.success) return fail("invalid_worker_input"); const operationNow = readOperationNow(runtime); if (!operationNow.ok || operationNow.epochMs > 8.64e15 - leaseDurationMs) return fail("invalid_runtime_time"); const candidate = [...jobs.values()].find((job) => job.status === "queued" || (job.status === "running" && Date.parse(job.leaseExpiresAt ?? "") <= operationNow.epochMs)); if (!candidate) return { ok: true, data: null, errorCode: null }; if (!isOperationTimeAtOrAfterJobTimeline(candidate, operationNow.epochMs)) return fail("invalid_runtime_time"); const attempt = candidate.attempt + 1; const leaseToken = `lease_v2_${fingerprint({ jobId: candidate.jobId, workerId: parsed.data.workerId, attempt, leasedAt: operationNow.iso })}` as const; const leased = signedJob({ ...candidate, status: "running", attempt, startedAt: candidate.startedAt ?? operationNow.iso, workerId: parsed.data.workerId, leaseToken, leasedAt: operationNow.iso, leaseExpiresAt: new Date(operationNow.epochMs + leaseDurationMs).toISOString(), resultIds: null, errorCode: null, completedAt: null, resultBindingIntegritySignature: null }); jobs.set(leased.jobId, leased); return { ok: true, data: clone(leased), errorCode: null }; } catch { return fail("invalid_worker_input"); } },
    async complete(input) { try { const parsed = completionSchema.safeParse(input); if (!parsed.success) return fail("invalid_completion_input"); const bundle = parseCanonicalResultBundle(parsed.data.resultBundle); if (!bundle) return fail("invalid_canonical_artifacts"); const operationNow = readOperationNow(runtime); if (!operationNow.ok) return fail("invalid_runtime_time"); const initialJob = jobs.get(parsed.data.jobId); if (!initialJob) return fail("lease_mismatch"); if (!isOperationTimeAtOrAfterJobTimeline(initialJob, operationNow.epochMs)) return fail("invalid_runtime_time"); if (!hasLeaseAuthority(initialJob, parsed.data, operationNow.epochMs)) return fail("lease_mismatch"); const persisted = await createStage2To7CanonicalArtifactValidatorV2(outcomeRepository).resolve(bundle, initialJob); if (!persisted) return fail("invalid_canonical_artifacts"); const currentJob = jobs.get(parsed.data.jobId); if (!currentJob || !hasLeaseAuthority(currentJob, parsed.data, operationNow.epochMs)) return fail("lease_mismatch"); const ids = resultIds(bundle, persisted); if (Object.values(ids).some((id) => !id)) return fail("invalid_canonical_artifacts"); const binding = resultBindingSignature(currentJob, bundle, persisted, ids); if (binding !== parsed.data.resultBindingIntegritySignature) return fail("result_binding_tampering"); const next = signedJob({ ...currentJob, status: "succeeded", completedAt: operationNow.iso, resultIds: ids, errorCode: null, resultBindingIntegritySignature: binding }); jobs.set(currentJob.jobId, next); return { ok: true, data: clone(next), errorCode: null }; } catch { return fail("invalid_completion_input"); } },
    async fail(input) { try { const parsed = z.object({ jobId: z.string().regex(/^async_simulation_job_v2_[a-f0-9]{24}$/), workerId: z.string().regex(/^worker_[a-z0-9][a-z0-9_-]*$/), leaseToken: z.string().regex(/^lease_v2_[a-f0-9]{24}$/), attempt: z.number().int().positive(), errorCode: z.string().min(1).max(200) }).strict().safeParse(input); if (!parsed.success) return fail("invalid_completion_input"); const operationNow = readOperationNow(runtime); if (!operationNow.ok) return fail("invalid_runtime_time"); const job = jobs.get(parsed.data.jobId); if (!job) return fail("lease_mismatch"); if (!isOperationTimeAtOrAfterJobTimeline(job, operationNow.epochMs)) return fail("invalid_runtime_time"); if (!hasLeaseAuthority(job, parsed.data, operationNow.epochMs)) return fail("lease_mismatch"); const next = signedJob({ ...job, status: "failed", completedAt: operationNow.iso, resultIds: null, errorCode: parsed.data.errorCode, resultBindingIntegritySignature: null }); jobs.set(job.jobId, next); return { ok: true, data: clone(next), errorCode: null }; } catch { return fail("invalid_completion_input"); } },
  };
}
