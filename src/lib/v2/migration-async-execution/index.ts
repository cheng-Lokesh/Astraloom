...
function timestampAt(epochMilliseconds: number) { return new Date(epochMilliseconds).toISOString(); }
...
function hasLeaseAuthority(job: AsyncSimulationJobV2 | undefined, input: { jobId: string; workerId: string; leaseToken: string; attempt: number }, runtime: AsyncExecutionRuntimeV2) {
  return Boolean(job && job.status === "running" && job.jobId === input.jobId && job.workerId === input.workerId && job.leaseToken === input.leaseToken && job.attempt === input.attempt && Date.parse(job.leaseExpiresAt ?? "") > runtime.nowEpochMs());
}
...
export function createInMemoryAsyncSimulationJobRepositoryV2(outcomeRepository: OutcomeCalibrationRepositoryPortV2, runtime: AsyncExecutionRuntimeV2): AsyncSimulationJobRepositoryPortV2 {
  ...
  return {
    ...
    async claim(input) { try { const parsed = workerSchema.safeParse(input); if (!parsed.success) return fail("invalid_worker_input"); const now = runtime.nowEpochMs(); const candidate = [...jobs.values()].find((job) => job.status === "queued" || (job.status === "running" && Date.parse(job.leaseExpiresAt ?? "") <= now)); if (!candidate) return { ok: true, data: null, errorCode: null }; const attempt = candidate.attempt + 1; const leasedAt = timestampAt(now); const leaseToken = `lease_v2_${fingerprint({ jobId: candidate.jobId, workerId: parsed.data.workerId, attempt, leasedAt })}` as const; const leased = signedJob({ ...candidate, status: "running", attempt, startedAt: candidate.startedAt ?? leasedAt, workerId: parsed.data.workerId, leaseToken, leasedAt, leaseExpiresAt: timestampAt(now + leaseDurationMs), resultIds: null, errorCode: null, completedAt: null, resultBindingIntegritySignature: null }); jobs.set(leased.jobId, leased); return { ok: true, data: clone(leased), errorCode: null }; } catch { return fail("invalid_worker_input"); } },
    async complete(input) { try { const parsed = completionSchema.safeParse(input); if (!parsed.success) return fail("invalid_completion_input"); const bundle = parseCanonicalResultBundle(parsed.data.resultBundle); if (!bundle) return fail("invalid_canonical_artifacts"); const initialJob = jobs.get(parsed.data.jobId); if (!initialJob || !hasLeaseAuthority(initialJob, parsed.data, runtime)) return fail("lease_mismatch"); const persisted = await createStage2To7CanonicalArtifactValidatorV2(outcomeRepository).resolve(bundle, initialJob); if (!persisted) return fail("invalid_canonical_artifacts"); const currentJob = jobs.get(parsed.data.jobId); if (!currentJob || !hasLeaseAuthority(currentJob, parsed.data, runtime)) return fail("lease_mismatch"); const ids = resultIds(bundle, persisted); if (Object.values(ids).some((id) => !id)) return fail("invalid_canonical_artifacts"); const binding = resultBindingSignature(currentJob, bundle, persisted, ids); if (binding !== parsed.data.resultBindingIntegritySignature) return fail("result_binding_tampering"); const next = signedJob({ ...currentJob, status: "succeeded", completedAt: timestampAt(runtime.nowEpochMs()), resultIds: ids, errorCode: null, resultBindingIntegritySignature: binding }); jobs.set(currentJob.jobId, next); return { ok: true, data: clone(next), errorCode: null }; } catch { return fail("invalid_completion_input"); } },
    async fail(input) { try { const parsed = z.object({ jobId: z.string().regex(/^async_simulation_job_v2_[a-f0-9]{24}$/), workerId: z.string().regex(/^worker_[a-z0-9][a-z0-9_-]*$/), leaseToken: z.string().regex(/^lease_v2_[a-f0-9]{24}$/), attempt: z.number().int().positive(), errorCode: z.string().min(1).max(200) }).strict().safeParse(input); if (!parsed.success) return fail("invalid_completion_input"); const job = jobs.get(parsed.data.jobId); if (!hasLeaseAuthority(job, parsed.data, runtime)) return fail("lease_mismatch"); const next = signedJob({ ...job!, status: "failed", completedAt: timestampAt(runtime.nowEpochMs()), resultIds: null, errorCode: parsed.data.errorCode, resultBindingIntegritySignature: null }); jobs.set(job!.jobId, next); return { ok: true, data: clone(next), errorCode: null }; } catch { return fail("invalid_completion_input"); } },
  };
}
