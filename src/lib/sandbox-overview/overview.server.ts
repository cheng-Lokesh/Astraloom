...
export async function readSandboxOverview(supabase: SupabaseClient, ownerId: string): Promise<SandboxOverview> {
  ...
  const [{ count: confirmedPeopleCount, error: peopleError }, { data: snapshotRecord, error: snapshotError }, { data: graphRecord, error: graphError }, { data: runningRecord, error: runningError }, { data: completedRecord, error: completedError }, { count: historyCount, error: historyError }, { data: feedbackRecord, error: feedbackError }] = await Promise.all([
    ...
    supabase.from("simulations").select("id,status,completed_at").eq("user_id", ownerId).eq("execution_version", "formal-account-sandbox-m1-v1").in("status", ["queued", "running"]).order("created_at", { ascending: false }).limit(1).maybeSingle(),
    supabase.from("simulations").select("id,status,completed_at").eq("user_id", ownerId).eq("execution_version", "formal-account-sandbox-m1-v1").eq("status", "completed").order("completed_at", { ascending: false }).limit(1).maybeSingle(),
    ...
    supabase.from("feedback_logs").select("id").eq("user_id", ownerId).eq("version", "formal-run-feedback-m1-v1").order("created_at", { ascending: false }).limit(1).maybeSingle(),
  ]);
  if (peopleError || snapshotError || graphError || runningError || completedError || historyError || feedbackError) throw new Error("sandbox_overview_read_failed");
