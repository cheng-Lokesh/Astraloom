...
const copy = {
  en: {
    title: "Persistence authorization archive review no-go reconciliation",
    badge: "Reconciliation checklist only",
    body: "This checklist reconciles whether the archive remediation review no-go packet is complete, traceable, redacted, and still blocked. It remains read-only and cannot accept no-go outcomes, mark reconciliation complete, deny or grant authorization, or write data.",
    yes: "Yes",
    no: "No",
    mode: "Mode",
    ...
    externalUnresolved: "External evidence unresolved",
    manualUnresolved: "Manual reviewer unresolved",
    stillBlocked: "Still blocked",
    sourceItems: "Source no-go items",
    sourceArchiveNoGo: "Source archive review no-go",
    sourceExternal: "Source external no-go",
    sourceManual: "Source manual no-go",
    traceability: "Traceability checks",
    blockerConsistency: "Blocker consistency",
    redaction: "Redaction checks",
    rejection: "Rejection triggers",
    unresolvedEvidence: "Unresolved evidence",
    forbiddenConclusions: "Forbidden conclusions",
  ...
  zh: {
    title: "持久化授权归档复核 No-go Reconciliation",
    badge: "仅 reconciliation 清单",
    body: "这份清单核对归档补救复核 No-go 包是否完整、可追溯、已脱敏且仍然阻断。它保持只读，不能接受 No-go 结果、标记 reconciliation 完成、拒绝或授予授权，也不能写入数据。",
    yes: "是",
    no: "否",
    mode: "模式",
    sourceMode: "来源 No-go 模式",
    checkedAt: "检查时间",
    reconciliationItems: "Reconciliation 项",
    externalUnresolved: "外部证据未解决",
    manualUnresolved: "人工复核未解决",
    stillBlocked: "仍然阻断",
    sourceItems: "来源 No-go 项",
    sourceArchiveNoGo: "来源归档复核 No-go",
    sourceExternal: "来源外部证据 No-go",
    sourceManual: "来源人工复核 No-go",
    traceability: "可追溯检查",
    blockerConsistency: "阻断一致性",
    redaction: "脱敏检查",
    rejection: "拒绝触发",
    unresolvedEvidence: "未解决证据",
    forbiddenConclusions: "禁止结论",
    futureInputs: "后续输入",
    ...
    probing: "检查中",
    probePanel: "阻断探测结果",
    probeBody: "探测一个 reconciliation 项，确认该清单仍然只读并保持阻断。",
...
function ReconciliationItemCard({
  ...
  item: WriterPersistenceAuthorizationReconsiderationFinalDecisionArchiveRemediationReviewNoGoReconciliationItem;
  copy: Copy;
  ...
  return (
    <article className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        ...
        <section>
          <h4 className="text-sm font-semibold text-slate-950">
            {copy.unresolvedEvidence}
          </h4>
          <TextList items={item.unresolvedEvidence} />
        </section>
        <section>
          <h4 className="text-sm font-semibold text-slate-950">
            {copy.forbiddenConclusions}
          </h4>
...
export function WriterPersistenceAuthorizationReconsiderationFinalDecisionArchiveRemediationReviewNoGoReconciliationClientPage({
  payload,
}: ClientPageProps) {
  const { locale } = useLanguage();
  const t = copy[locale];
  const [probeResult, setProbeResult] =
    useState<WriterPersistenceAuthorizationReconsiderationFinalDecisionArchiveRemediationReviewNoGoReconciliationProbeResult | null>(
      null,
    );
  ...
  const trueFlags = [
    {
      value:
        payload.externalFinalDecisionArchiveRemediationReviewNoGoReconciliationChecklistReady,
      label:
        "externalFinalDecisionArchiveRemediationReviewNoGoReconciliationChecklistReady",
    },
    {
      value:
        payload.externalFinalDecisionArchiveRemediationReviewNoGoReconciliationChecklistOnly,
      label:
        "externalFinalDecisionArchiveRemediationReviewNoGoReconciliationChecklistOnly",
    },
    {
      value:
        payload.sourceExternalFinalDecisionArchiveRemediationReviewNoGoPacketReady,
      label:
        "sourceExternalFinalDecisionArchiveRemediationReviewNoGoPacketReady",
    },
    {
      value:
        payload.sourceExternalFinalDecisionArchiveRemediationReviewNoGoPacketOnly,
      label:
  ...
  const runtimeFlags = [
    ...
    {
      value:
        payload.wouldRecordFinalDecisionArchiveRemediationReviewNoGoReconciliation,
      label:
        "wouldRecordFinalDecisionArchiveRemediationReviewNoGoReconciliation",
    },
    {
      value: payload.wouldMarkArchiveRemediationReviewNoGoReconciled,
      label: "wouldMarkArchiveRemediationReviewNoGoReconciled",
    },
    {
      value:
        payload.wouldPromoteArchiveRemediationReviewNoGoReconciliationToAuthorizationDecision,
      label:
    ...
    {
      value: payload.wouldDenyImplementationAuthorizationFromArchiveRemediationReview,
  ...
  async function probeItem(itemId: string) {
    ...
    try {
      ...
      const result =
        (await response.json()) as WriterPersistenceAuthorizationReconsiderationFinalDecisionArchiveRemediationReviewNoGoReconciliationProbeResult;
    ...
    <AppShell>
      ...
      <section className="mb-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <Stat
          label={t.mode}
          value={
            payload.externalFinalDecisionArchiveRemediationReviewNoGoReconciliationChecklistMode
          }
        />
        <Stat
          label={t.sourceMode}
          value={payload.sourceExternalFinalDecisionArchiveRemediationReviewNoGoMode}
        />
        <Stat
          label={t.reconciliationItems}
          value={payload.reconciliationItemCount}
        />
        <Stat
        ...
        <Stat
          ...
          value={payload.archiveReviewNoGoStillBlockedCount}
        />
        <Stat label={t.sourceItems} value={payload.sourceNoGoItemCount} />
        <Stat
          label={t.sourceArchiveNoGo}
          value={payload.sourceArchiveReviewNoGoCount}
        />
        <Stat
          label={t.sourceExternal}
          value={payload.sourceExternalEvidenceNoGoCount}
        />
        <Stat
          label={t.sourceManual}
          value={payload.sourceManualReviewerNoGoCount}
        />
        <Stat label={t.traceability} value={payload.traceabilityCheckCount} />
        <Stat
          label={t.blockerConsistency}
          value={payload.blockerConsistencyCheckCount}
        />
        <Stat label={t.redaction} value={payload.redactionCheckCount} />
        <Stat label={t.rejection} value={payload.rejectionTriggerCount} />
        <Stat
          label={t.unresolvedEvidence}
          value={payload.unresolvedEvidenceCount}
        />
        <Stat
          label={t.forbiddenConclusions}
          value={payload.forbiddenConclusionCount}
        />
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px]">
        <section className="space-y-5">
          <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <TextList
              items={payload.archiveRemediationReviewNoGoReconciliationRules}
            />
          </section>

          {payload.archiveRemediationReviewNoGoReconciliationItems.map(
            (item) => (
        ...
        <aside className="space-y-5">
          ...
          <section className="rounded-lg border border-rose-200 bg-rose-50 p-5 shadow-sm">
            ...
            <TextList
              items={
                payload.archiveRemediationReviewNoGoReconciliationRejectionRules
              }
            />
          </section>
            ...
            {probeResult ? (
              <div className="mt-4 rounded-md border border-slate-200 bg-slate-50 p-4 text-sm leading-6 text-slate-700">
                ...
                <p>
                  {t.mode}:{" "}
                  {
                    probeResult.externalFinalDecisionArchiveRemediationReviewNoGoReconciliationChecklistMode
                  }
                </p>
