...
const copy = {
  en: {
    title: "Persistence authorization archive reconciliation remediation review no-go",
    badge: "Review no-go packet only",
    body: "This packet explains why the archive remediation review no-go reconciliation remediation review still cannot unlock implementation authorization. It remains read-only and cannot accept no-go outcomes, deny authorization, grant authorization, start implementation, or write rows.",
    yes: "Yes",
    no: "No",
    mode: "Mode",
    ...
    sourceBlocked: "Source review blocked",
  ...
  zh: {
    title: "持久化授权归档 reconciliation remediation 复核 No-go",
    badge: "仅复核 No-go 包",
    body: "这份包说明归档补救复核 no-go reconciliation remediation 复核为什么仍不能解锁实现授权。它保持只读，不能接受 no-go 结果、拒绝授权、授予授权、启动实现或写入数据。",
    yes: "是",
    no: "否",
    mode: "模式",
    sourceMode: "来源复核模式",
    ...
    probing: "检查中",
    probePanel: "阻断探测结果",
    probeBody: "探测一个 No-go 项，确认这份包仍然只读且保持阻断。",
...
function NoGoItemCard({
  ...
  item: WriterPersistenceAuthorizationReconsiderationFinalDecisionArchiveRemediationReviewNoGoReconciliationRemediationReviewNoGoItem;
  copy: Copy;
...
export function WriterPersistenceAuthorizationReconsiderationFinalDecisionArchiveRemediationReviewNoGoReconciliationRemediationReviewNoGoClientPage({
  payload,
}: ClientPageProps) {
  const { locale } = useLanguage();
  const t = copy[locale];
  const [probeResult, setProbeResult] =
    useState<WriterPersistenceAuthorizationReconsiderationFinalDecisionArchiveRemediationReviewNoGoReconciliationRemediationReviewNoGoProbeResult | null>(
      null,
    );
  ...
  const trueFlags = [
    {
      value:
        payload.externalFinalDecisionArchiveRemediationReviewNoGoReconciliationRemediationReviewNoGoPacketReady,
      label:
        "externalFinalDecisionArchiveRemediationReviewNoGoReconciliationRemediationReviewNoGoPacketReady",
    },
    {
      value:
        payload.externalFinalDecisionArchiveRemediationReviewNoGoReconciliationRemediationReviewNoGoPacketOnly,
      label:
        "externalFinalDecisionArchiveRemediationReviewNoGoReconciliationRemediationReviewNoGoPacketOnly",
    },
    {
      value:
        payload.sourceExternalFinalDecisionArchiveRemediationReviewNoGoReconciliationRemediationReviewChecklistReady,
      label:
        "sourceExternalFinalDecisionArchiveRemediationReviewNoGoReconciliationRemediationReviewChecklistReady",
    },
    {
      value:
        payload.sourceExternalFinalDecisionArchiveRemediationReviewNoGoReconciliationRemediationReviewChecklistOnly,
      label:
  ...
  const runtimeFlags = [
    ...
    {
      value:
        payload.wouldRecordFinalDecisionArchiveRemediationReviewNoGoReconciliationRemediationReviewNoGo,
      label:
        "wouldRecordFinalDecisionArchiveRemediationReviewNoGoReconciliationRemediationReviewNoGo",
    },
    {
      value:
        payload.wouldDenyImplementationAuthorizationFromArchiveRemediationReviewNoGoReconciliationRemediationReview,
      label:
        "wouldDenyImplementationAuthorizationFromArchiveRemediationReviewNoGoReconciliationRemediationReview",
    },
    {
      value:
        payload.wouldPromoteArchiveRemediationReviewNoGoReconciliationRemediationReviewNoGoToAuthorizationDecision,
      label:
  ...
  async function probeItem(itemId: string) {
    ...
    try {
      ...
      const result =
        (await response.json()) as WriterPersistenceAuthorizationReconsiderationFinalDecisionArchiveRemediationReviewNoGoReconciliationRemediationReviewNoGoProbeResult;
    ...
    <AppShell>
      ...
      <section className="mb-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <Stat
          label={t.mode}
          value={
            payload.externalFinalDecisionArchiveRemediationReviewNoGoReconciliationRemediationReviewNoGoMode
          }
        />
        <Stat
          label={t.sourceMode}
          value={
            payload.sourceExternalFinalDecisionArchiveRemediationReviewNoGoReconciliationRemediationReviewChecklistMode
          }
        />
        <Stat label={t.noGoItems} value={payload.noGoItemCount} />
        <Stat label={t.reviewNoGo} value={payload.reviewNoGoCount} />
        <Stat
        ...
        <Stat
          ...
          value={payload.reconciliationRemediationReviewStillBlockedCount}
        />
        ...
        <Stat
          ...
          value={payload.sourceReconciliationRemediationStillBlockedCount}
        />
        <Stat
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px]">
        <section className="space-y-5">
          <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <TextList
              items={
                payload.archiveRemediationReviewNoGoReconciliationRemediationReviewNoGoRules
              }
            />
          </section>

          {payload.archiveRemediationReviewNoGoReconciliationRemediationReviewNoGoItems.map(
            (item) => (
              <NoGoItemCard
                key={item.id}
                item={item}
        ...
        <aside className="space-y-5">
          ...
          <section className="rounded-lg border border-rose-200 bg-rose-50 p-5 shadow-sm">
            ...
            <TextList
              items={
                payload.archiveRemediationReviewNoGoReconciliationRemediationReviewNoGoRejectionRules
              }
            />
          </section>
            ...
            {probeResult ? (
              <div className="mt-4 rounded-md border border-slate-200 bg-slate-50 p-4 text-sm leading-6 text-slate-700">
                ...
                <p>
                  {t.mode}:{" "}
                  {
                    probeResult.externalFinalDecisionArchiveRemediationReviewNoGoReconciliationRemediationReviewNoGoMode
                  }
                </p>
