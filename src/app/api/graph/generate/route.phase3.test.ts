...
describe("POST /api/graph/generate", () => {
  ...
  it("returns 200 with the same complete locked Graph when generation replays after lock", async () => {
    ...
    const response = await POST(request({ selector: { seed_id: seedId }, idempotency_key: idempotencyKey }));
    expect(response.status).toBe(200);
    expect(await response.json()).toMatchObject({ ok: true, idempotent: true, graph: { id: graphId, graph_locked: true, locked_at: lockedAt }, edges: [{ graph_snapshot_id: graphId }] });
  });
