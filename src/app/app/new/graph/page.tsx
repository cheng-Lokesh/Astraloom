...
export default function GraphPage() {
  ...
  const sync = useCallback(() => setState({ ...controller.state }), [controller]);
...
function Blocked({ state, reload }: { state: FormalGraphState; reload: () => void }) {
  return <ReadOnlyHold state={state} reload={reload} title="Graph action is blocked" description="The saved safety boundary did not permit this action. No Graph snapshot was written, and later writes are disabled until the account ledger is refreshed." />;
}

function ReadOnlyHold({ state, reload, title, description }: { state: FormalGraphState; reload: () => void; title: string; description: string }) {
