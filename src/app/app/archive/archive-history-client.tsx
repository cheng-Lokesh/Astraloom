...
function completedRuns(items: Run[]) {
  return items.filter((run) => run.status === "completed");
}

function formatRunTime(run: Run, timeUnavailableLabel: string) {
  const timestamp = run.completed_at ?? run.created_at;
  return timestamp ? new Date(timestamp).toLocaleString() : timeUnavailableLabel;
}
